...
## 設定バインディング・エラーハンドリング・エンドポイント構成のリファクタリング

後続のパスで、「dotnet固有の実装についてはフレームワーク推奨の実装を優先する」——たとえ手作りの
代替案の方が技術的には「より最小限」に見えても——という直接の要望を受け、プロジェクト自身の規約を
見直しました。そこから3つの変更が生まれました。

- **`CompanionSettings.FromEnvironment()`（素の`Environment.GetEnvironmentVariable`呼び出し）が
  `IConfiguration.Get<CompanionSettings>()`に変わりました**——起動時に一度だけ読む設定をPOCOへ
  バインドする.NET標準の仕組みです。`CompanionSettings`は位置引数の`record`から、setter付き
  プロパティを持つクラスへ変更しました（バインダーはパラメータ無しコンストラクタとsetter付き
  プロパティを必要とします）。各プロパティには`[ConfigurationKeyName("LINE_...")]`を付与し、
  バインド先のキーが元の環境変数名そのまま（フラットなキー）になるようにしています——
  `LINE_CHANNEL_SECRET`は今まで通り同じ意味・同じ設定方法・同じ`Has*`による未設定時のゲート挙動
  のままです。より重量級の`IOptions<T>`/`Configure<T>()`によるOptionsパターンではなく
  `IConfiguration.Get<T>()`を選んだのは意図的な判断です:このアプリは設定のホットリロードも
  起動時バリデーションも必要とせず（後者は「未設定でも常に起動する」という設計方針と衝突します）、
  同じく標準的でありながらより軽量な`Get<T>()`バインディングの方が、誰も使わない仕組みを増やさず
  収まりが良いためです。1点、挙動の変化として知っておくべきこと: `LINE_MINIAPP_POLL_SECONDS`に
  パース不能な値（数値以外の文字列など）を設定すると、以前は`int.TryParse`が黙って30へフォール
  バックしていましたが、現在は初回アクセス時に例外を送出します——これはまさに、以前は隠蔽されて
  いた操作ミスを、そのまま大きな音で表面化させる変更です。
  - 1回限りの`dotnet run -- setup`CLIパスは`WebApplication`/ホストが構築される前に実行されるため、
    再利用できる`IConfiguration`がありません——現在はWebホストと同じプロバイダ構成を模した
    `new ConfigurationBuilder().AddJsonFile("appsettings.json", optional: true).AddEnvironmentVariables().Build()`
    で自前に組み立てています。
  - `appsettings.json`/`appsettings.Development.json`を、ASP.NET CoreのWebテンプレートが標準で
    同梱する`Logging`セクション付きで追加しました——このアプリの`LINE_*`設定は実運用上引き続き
    環境変数から取得しますが、標準的な設定ファイルの形が「無い」状態から「ある」状態になり、
    環境変数無しでログレベルを調整できるという実用上のメリットもあります。
  - インメモリの`IConfiguration`を組み立てて`[ConfigurationKeyName]`のバインディングが正しく
    往復することを検証するテスト（`CompanionSettingsBindingTests`）を書く過程で、実際のギャップを
    1つ発見しました: `IConfiguration.Get<T>()`は、設定が完全に空の場合、デフォルト値で埋めた
    インスタンスではなく`null`を返します——これが`Program.cs`に`?? new CompanionSettings()`という
    フォールバックがある理由であり、単なる過剰な防御ではなかったということです。
- **`builder.Services.AddProblemDetails()` + `app.UseExceptionHandler()`を追加。** 未処理例外の
  応答を素の500ではなく`application/problem+json`形式に整形する、.NET 8以降の標準パターンです。
  既知のエラー条件については各`Results.Problem(...)`呼び出しが既にこの形状を返していましたが、
  これは本当に予期しない例外の側のギャップを埋めます（以前はDevelopment例外ページや素の500に
  当たっていたことを生のリクエストで確認済み——上記のスモークテストで返っている503応答に
  `traceId`フィールドが含まれているのは、`AddProblemDetails()`が応答を拡張している証拠です）。
- **`/webhook`と`/api/shop/*`のハンドラを`Program.cs`から切り出し**、`Endpoints/WebhookEndpoints.cs`
  と`Endpoints/ShopEndpoints.cs`（拡張メソッド`MapWebhookEndpoint()`/`MapShopEndpoints()`、後者は
  `MapGroup("/api/shop")`を使用）に移動しました——1つの`Program.cs`に複数の本格的なハンドラが
  積み上がってきた段階でルートを整理する、minimal API自身が推奨するパターンです。これに伴い
  小さな整理も必要になりました: ハンドラは（別ファイル・別メソッドになったため）もう`Program.cs`の
  ローカル変数`settings`をクロージャで捕捉できなくなり、他の箇所と同様に`CompanionSettings`を
  DI解決されるパラメータとして受け取るようになりました——一部のコードパスはDI、他はクロージャ
  捕捉、という不整合を解消しています。1行だけの些細な`/`ステータスエンドポイントは`Program.cs`に
  そのまま残し、切り出す価値のある本格的な2つのハンドラだけを移動しました。

このリファクタリングが存在するのは、プロジェクト自身の設計記録に「反して」ではなく、むしろ
「それを踏まえて」のことです: 直前のリファクタリング（上記の永続化の抽象化）のレビューゲートで、
当時の`CLAUDE.md`の規約との矛盾が指摘され、その場ではその抽象化1つだけを明示的な例外として
狭く追記する形で対応していました。その場限りの例外を積み重ねるのではなく、`CLAUDE.md`の規約
セクションはその後、本リファクタリングが実装しているより広い優先順位——dotnet/ASP.NET固有の
実装はフレームワーク推奨のアプローチをデフォルトとする、単純化によってASP.NET CoreのWebアプリ
としての標準的な形を崩してはならない——を明示する形に直接書き換えられました。これは「
`appsettings.json`バインドは使わず素の環境変数読み込み」「呼び出し元1箇所しかない抽象化は避ける」
という古い行の隣に例外を増やすのではなく、それらを丸ごと置き換えるものです。
