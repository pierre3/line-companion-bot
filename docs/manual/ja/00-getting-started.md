...
> ローカル検証できます。もし作り*ながら*返信やリッチメニューを自分の実機で見たいなら、Messaging API
> チャネル＋アクセストークンとdev tunnelを先に用意し（コンソールとトンネルの手順は[第9章](09-end-to-end.md)）、
> 第2章以降はチャネルのWebhookを自分のトンネルに向けてください。注意が2つ：ショップ/IAP側は審査ゲートの
> あるMINI Appチャネルが必要（第6/9章）で、ブレークポイントで止めると約1分のリプライトークンがカード
> 送信前に失効することがあります。
