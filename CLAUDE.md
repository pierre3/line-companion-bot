...
ハンズオンマニュアル（`docs/manual/{en,ja}/`、章ごとに1ファイル分割・索引は各言語の `README.md`、
実装ステップと1:1対応、bilingual、VSCodeでのF5実行/デバッグとuser-secretsを前提）。実装ステップを
完了する都度、対応する章を書く（まとめて後書きしない）。
...
`docs/manual/{en,ja}/` の対応章ファイル（`0N-*.md`）を書く。**各ステップの完了ごとにビルド/テストを確認**し、
